...
!(function () {
  ...
  window.addEventListener('touchend', function (e) {
    ...
    if (activeTab.id == "c_item2") {
      ...
      document.getElementById("tabs").style.backgroundColor = "rgba(0, 0, 0, 0.7)";
      map_obj.classList.remove('map-zoom');
    }
    else {
      if (targetTab.id == "c_item2") {
        map_obj.style.display = "block";
        document.getElementById("tabs").style.backgroundColor = "rgba(0, 0, 0, 0.5)";
        //debugger;
        document.getElementById("item1mobile").classList.remove('opaco');
        document.getElementById("item3mobile").classList.remove('opaco');
        setTimeout(function(){ map_obj.classList.add('map-zoom'); }, 100);
        
        setTimeout(function(){ mappa.invalidateSize(); }, 500);
    ...
    if (targetTab.id != "c_item2") {
      ...
      if (activeTab.id == "c_item2") {
        setTimeout(function(){ document.getElementById("card").scrollTop = 0; }, 50); 
        //document.getElementById("item3mobile").classList.remove('opaco');
      }           
    }
    ...
    for (var i = 0; i < activeBodies.length; i++) {
      activeBodies[i].classList.remove('opaco');
      activeBodies[i].classList.add(className);
    }

    targetBody.classList.add(className);
    
    targetBody.classList.add('opaco');
  });
